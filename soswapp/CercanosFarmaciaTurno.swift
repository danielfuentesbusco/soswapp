...
class CercanosFarmaciasTurnoController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let userLat = defaults.stringForKey("userLat") {
            if let userLng = defaults.stringForKey("userLng") {
                var url : String = "http://soswapp.cl/api/farmacias-emergencia.php?s=5&a=" + userLat + "&n=" + userLng
                var cant = 0;
    ...
    func mapView(mapView: MKMapView, viewForAnnotation annotation: MKAnnotation) -> MKAnnotationView? {
        if let annotation = annotation as? CustomAnnot {
            ...
            if let dequeuedView = mapView.dequeueReusableAnnotationViewWithIdentifier(identifier) { // 2
                    ...
                    view = dequeuedView
                    view.image=UIImage(named:"pinfarma.png")
    ...
    func mapView(mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        if control==view.rightCalloutAccessoryView {
            if let data=view.annotation as? CustomAnnot{
                ...
                let alertController = UIAlertController(title: "SOSWApp", message: "Esta Farmacia de Turno está a "+data.distancia+"km de usted y está abierta las 24 horas del día.", preferredStyle: UIAlertControllerStyle.Alert)
