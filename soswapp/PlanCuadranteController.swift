...
class PlanCuadranteController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let userLat = defaults.stringForKey("userLat") {
            if let userLng = defaults.stringForKey("userLng") {
                var url : String = "http://soswapp.cl/api/servicios-cercanos.php?s=7&a=" + userLat + "&n=" + userLng
                var cant = 0;
    ...
    func mapView(mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        if control==view.rightCalloutAccessoryView {
            if let data=view.annotation as? CustomAnnot{
                ...
                let alertController = UIAlertController(title: "SOSWApp", message: data.title!+" de la " + data.locationName, preferredStyle: UIAlertControllerStyle.Alert)
