...
class CercanosFarmaciasController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let userLat = defaults.stringForKey("userLat") {
            if let userLng = defaults.stringForKey("userLng") {
                var url : String = "http://soswapp.cl/api/servicios-cercanos.php?s=5&a=" + userLat + "&n=" + userLng
                var cant = 0;
    ...
    func mapView(mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        if control==view.rightCalloutAccessoryView {
            if let data=view.annotation as? CustomAnnot{
                ...
                let alertController = UIAlertController(title: "SOSWApp", message: "Esta Farmacia está a "+data.distancia+"km de usted.", preferredStyle: UIAlertControllerStyle.Alert)
